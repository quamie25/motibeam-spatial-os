...
class TransportRealm:
    ...
    def draw_frame(self):
        """Draw the transport realm frame"""
        # Dark green background
        self.screen.fill((20, 40, 30))

        # Title
        title = self.font_title.render("🚗 TRANSPORT REALM", True, (180, 255, 200))
        self.screen.blit(title, (50, 50))

        # Mode display
        mode = getattr(self.global_state, "mode", "NORMAL")
        mode_text = self.font_body.render(f"Mode: {mode}", True, (140, 220, 160))
        self.screen.blit(mode_text, (50, 120))

        # Instructions
        instructions = self.font_body.render("Press ESC to exit", True, (120, 190, 140))
